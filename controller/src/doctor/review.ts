...
function renderSettingsSnapshot(): string {
  ...
  return [
    ...
    `- Budget: ${budget} · pauseWhenEmpty ${llm.pauseWhenEmpty ? 'ON' : 'OFF'}`,
    `- Picking: noRepeatWindow ${llm.noRepeatWindow ?? 'n/a'}`,
    `- Broadcast: handover autocue · jingle 1/${s?.jingleRatio ?? '?'} (${s?.jingleRotate === 'controller' ? 'controller' : 'mixer'}) · maxTrack ${s?.maxTrackSeconds ? s.maxTrackSeconds + 's' : 'unlimited'} · mounts ${mounts.join('+')} · archive ${s?.archive?.enabled ? 'ON' : 'OFF'}`,
