...
- **Optional adaptive HLS.** `/hls/live.m3u8` beside the Icecast mounts: four AAC rungs (320–128 kbps) the player steps between as the connection changes, resuming by fetching the next segment instead of reconnecting — built for patchy mobile listening. HLS listeners are counted alongside Icecast ones.
